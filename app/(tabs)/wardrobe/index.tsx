...
export default function WardrobeScreen() {
  ...
  const cols = 2;
  const card = Math.floor((width - horizontalPadding * 2 - gap * (cols - 1)) / cols);
  ...
  const askDelete = (id: string) => {
    Alert.alert(
      t('alerts.deleteLookTitle'),
      t('alerts.deleteLookText'),
      [
    ...
    >
      {items.length === 0 && !isLoading ? (
        <View style={s.empty}>
          ...
          <Pressable style={s.primaryBtn} onPress={() => router.push('/(tabs)/try-on')}>
          ...
          {items.map(item => (
            ...
            >
              ...
              <View style={s.overlay}>
                <Text numberOfLines={1} style={s.date}>{formatDate(item.createdAt)}</Text>
              </View>
            </Pressable>
          ))}
        </View>
...
const s = StyleSheet.create({
  ...
  primaryBtn: { marginTop: 8, backgroundColor: '#111827', paddingVertical: 12, paddingHorizontal: 16, borderRadius: 12 },
