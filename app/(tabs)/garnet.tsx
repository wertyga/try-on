...
import {
  View, Text, Pressable, Image, Alert, ActivityIndicator, Platform, StyleSheet
} from 'react-native';
...
export default function Garnet() {
  ...
  const {
    mode, setMode,
    dress, upper, lower,
    setDress, setUpper, setLower,
    clearDress, clearUpper, clearLower,
  } = useTryOnStore();
  
  const [busy, setBusy] = useState(false);
  
  async function pick(setter: (x: { uri: string; base64: string } | null) => void) {
    try {
      ...
      setter({ uri: manip.uri, base64: `data:image/jpeg;base64,${manip.base64!}` });
    } catch (e: any) {
      Alert.alert(t('common.error'), e?.message || t('errors.pickImageFallback'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Container.WithScrollBar>
      ...
      <View style={s.switchRow}>
        ...
        >
          <Text style={[s.switchText, mode === 'separate' && s.switchTextActive]}>
      ...
      {mode === 'dress' ? (
        ...
        <>
          <Card
            ...
            imageUri={upper?.uri}
            onPick={() => pick(setUpper)}
            onClear={clearUpper}
          />
          <Card
            title={t('garnet.cardBottom')}
            imageUri={lower?.uri}
            onPick={() => pick(setLower)}
            onClear={clearLower}
          />
        </>
      )}
...
function Card({
  title, imageUri, onPick, onClear,
}: { title: string; imageUri?: string; onPick: () => void; onClear: () => void; }) {
  const { t } = useTranslation();
  return (
    <View style={s.card}>
...
const s = StyleSheet.create({
  ...
  switchBtn: { flex: 1, paddingVertical: 12, borderRadius: 12, backgroundColor: "#E5E7EB", alignItems: "center" },
  switchBtnActive: { backgroundColor: "#111827" },
  switchText: { color: "#111827", fontWeight: "700" },
  switchTextActive: { color: "#fff" },
  
  card: { backgroundColor: "#F9FAFB", borderRadius: 16, padding: 12, marginBottom: 12 },
  cardTitle: { fontSize: 16, fontWeight: "700", marginBottom: 8 },
  muted: { color: "#6B7280" },
  
  previewFrame: { width: "100%", height: 360, borderRadius: 16, backgroundColor: "#E5E7EB", overflow: "hidden" },
  preview: { width: "100%", height: "100%" },
  
  primaryBtn: { flex: 1, backgroundColor: "#111827", paddingVertical: 12, borderRadius: 12, alignItems: "center" },
  primaryBtnText: { color: "#fff", fontWeight: "700" },
  clearBtn: { backgroundColor: "#FFE4E6", paddingVertical: 12, borderRadius: 12, alignItems: "center", flex: 1 },
