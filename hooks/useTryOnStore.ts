...
export type UserPhoto = {
	uri: string; base64?: string; width?: number; height?: number; source?: Source;
} | null;

export type GarmentImage = { uri: string; base64?: string } | null;
export type GarmentMode = "dress" | "separate";

export type TTryOnPayloadWardrobeKeys = 'dressBase64' | 'upperBase64' | 'lowerBase64';

export type TryOnPayload = Partial<Record<TTryOnPayloadWardrobeKeys, string>> & {
	mode: GarmentMode;
	userBase64: string;
};
...
export const useTryOnStore = create<TryOnState>((set) => ({
	...
	setMode: (m) =>
		set((state) =>
			m === "dress" ? { mode: m, upper: null, lower: null } : { mode: m, dress: null }
		),
	setDress: (dress) => set({ dress }),
	setUpper: (upper) => set({ upper }),
	setLower: (lower) => set({ lower }),
	resetInputs: () => set({ dress: null, upper: null, lower: null, /* userPhoto оставим */ }),
	...
	clearFinished: () =>
		set((s) => ({ tasks: s.tasks.filter((x) => x.status !== "completed" && x.status !== "failed") })),
}));
